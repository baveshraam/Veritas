...
| RAG-14 | `CRIME_SEARCH` | same | Yes (prior pass) | BROKEN — answers with narratives, never a count (BUG-008, deliberate) |
...
| ML-04 | XGBoost + SHAP risk scoring | `risk/scoring.py` | Yes: fires, returns a score + top contributors | PARTIAL — returns 1.00 (saturated), calibration unvalidated (BUG-014) |
