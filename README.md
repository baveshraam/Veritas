...
| *"How does he operate?"* | An **evidence-backed behavioral profile** — never demographic — built entirely from a person's own case history: recurring time-of-day, an exact repeated method, geographic range, and an escalation in offence severity where the record's own gravity classification actually shows one. Every line names the specific FIR numbers it was read from. |
| *"Is this part of a pattern?"* | **Unprompted cross-station series discovery** — the "linkage blindness" problem FBI ViCAP has named as its own weakness since 1985: a shared, exact modus-operandi match to open cases at *different* stations, with no common suspect yet on file, so nobody investigating any one of them would otherwise know the others exist. Surfaced proactively (an alert feed, not just an answer to a question asked) as well as conversationally. |
