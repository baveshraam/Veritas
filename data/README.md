...
## Relational + geospatial schema (Postgres — plain lat/lng, no PostGIS)

```sql
CREATE TABLE officer (
    officer_id UUID PRIMARY KEY, badge_no VARCHAR(20) UNIQUE,
...
CREATE TABLE fir (
    ...
    latitude DECIMAL(9,6), longitude DECIMAL(9,6),   -- CaseMaster.latitude/longitude in the KSP ER diagram
    district VARCHAR(50), taluk VARCHAR(50),
...
CREATE TABLE person (
    ...
    address_lat DECIMAL(9,6), address_lng DECIMAL(9,6), aadhaar_hash VARCHAR(64),
    ...
    pagerank FLOAT, community INT, betweenness FLOAT   -- written back by the NetworkX job (data.gds)
);
...
## Knowledge graph schema (edge-list table + NetworkX)

There is no graph database. No Catalyst service corresponds to one, so the graph is
stored as ordinary rows in `graph_edge` (full DDL in `sql/001_schema.sql`) and the
algorithms run in NetworkX over them (`data.graph`, `data.gds`).

Nodes *are* the records — `person`, `fir`, `account`, `txn` — plus two name-only kinds
(`Gang`, `Location`) that exist only as edge endpoints. Only the edges need a table:

| `edge_type` | src → dst | edge columns used |
|---|---|---|
| `ACCUSED_IN` | Person → CrimeEvent | `role`, `edge_date` |
| `VICTIM_IN` | Person → CrimeEvent | — |
| `CO_ACCUSED_WITH` | Person → Person | `strength` (shared FIR count) |
| `MEMBER_OF` | Person → Gang | `role`, `edge_date` |
| `OCCURRED_AT` | CrimeEvent → Location | — |
| `OWNS_ACCOUNT` | Person → Account | — |
| `TRANSFERRED_TO` | Account → Account | `amount`, `edge_date` |
| `INVOLVED_IN` | Account → Transaction | — |
| `LINKED_TO` | Transaction → CrimeEvent | — |
| `SAME_AS` | Person → Person | `confidence` (Fellegi-Sunter) |

`TRANSFERRED_TO` is the **only** directed relation — money moved one way and a trail that
walks it backwards is a false trail. Everything else is mirrored on load (`data.graph._SYMMETRIC`).

`data.gds` runs PageRank, Louvain, betweenness (pivot-sampled) and personalized PageRank
(the HippoRAG seed primitive) on demand, writing `pagerank`/`community`/`betweenness` back
onto `person`. `packages/rag_agent` reads those results; it doesn't run the algorithms itself.
...
3. Runs `packages/ml_models.resolve_entities()` once over all generated `person` rows as a batch dedup pass, writing `canonical_entity_id` and `SAME_AS` edges. **This is the only caller of `resolve_entities`** — it is a batch/offline operation (pairwise candidate scoring), not something `packages/rag_agent` invokes per query. At query time, "has this person been arrested under another name" is answered by reading the already-resolved `SAME_AS` edges straight out of `graph_edge`, not by recomputing linkage live.
4. (Re)builds Postgres, rebuilds the `graph_edge` list and reruns the graph algorithms, re-embeds narratives into the vector store.
...
```
data/
  sql/                 # Postgres DDL + migrations (no PostGIS: plain lat/lng)
